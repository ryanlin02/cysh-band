...
1. 將新照片資料夾放入 `嘉中管樂照片庫/05-照片集拷貝/`。
2. 雙擊 `嘉中管樂照片庫/07-照片網站/更新網站資料.command`，等待驗證與上傳全部完成。
3. 流程會重建完整公開人物頭像、建立新的版本化 runtime，再以 R2
   `runtime/bootstrap.json` 切換正式資料；上傳使用非刪除式 `rclone copy`，不得改回
   對 `data/` 執行 `rclone sync`。
4. 新照片若要支援畫面內容搜尋，先完成 `06-AI辨識工作區` 的新照片標註，再重跑一次更新指令。

整個 `photos/*` 路徑已被保護，新增的相簿、人物照片頁與搜尋結果會自動要求登入，不必逐張設定權限。
