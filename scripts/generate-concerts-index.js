...
const ARCHIVE_INTROS = {
  '2025-40th': '《四方之音》呼應校友來自四面八方、齊聚一堂共襄盛舉。第 40 屆由四字頭校友承接籌辦，簡晟軒擔任指揮，總召陳乃慎統籌演出行政；目前曲目、團員名單與更完整的節目冊資料仍待補齊。若校友保存海報、節目單、照片或錄音，歡迎協助補充，讓第 40 屆的紀錄更完整。',
  ...
  '2023-38th': '《一樹起響》呼應嘉中校園老樹意象，象徵眾人齊聚一堂、共同奏響樂章；副標 Saeculum illuminate 也為隔年的百年校慶揭開序幕。本屆由翁啟榮籌備統籌，盧宓承、丁肇賢、簡晟軒共同擔任指揮，並首演葉哲良為建校百年創作的《旭陵慶典》，把校歌旋律與「質實剛健」精神帶回聯演舞台。',
  ...
  '2018-34th': '《青春の極短篇》以「捌月季」與「當我的七仔好嗎？」為宣傳語，由曾膺安、簡晟軒擔任指揮，2018 年 8 月 5 日於嘉義市文化局音樂廳演出。曲目包含八木澤教司《稚鳥飛翔》、酒井格《七夕》、《豪勇七蛟龍》、007 組曲與岩井直溥編曲《美國風情畫》等；團員名單仍待補齊。',
  ...
  '2014-30th': '《三十而樂》紀念校友聯演傳統邁入第三十屆，副標「卅有其誓 × 出磊拔粹」延伸出屆次與青春記憶的雙關。現階段已保留主題與年度線索，但日期、場地、指揮、曲目與團員名單仍待補齊；若校友保存節目冊、海報、照片或錄音，歡迎協助補充。',
  ...
  '2011-27th': '第 27 屆聯合音樂會目前已能確認約於 2011 年舉行，日期線索暫列 7 月 16 日；場地、指揮、主題、曲目與團員名單仍待節目冊或海報補齊。同年另有世界管樂年會相關的校友演出紀錄，現階段先作為同年脈絡保存，不直接併入本屆。歡迎校友提供更多資料協助考證。',
  ...
  '2002-18th': '第 18 屆聯合音樂會於 2002 年 8 月 30 日在嘉義市音樂廳演出，是目前影像保存最完整的早期屆別之一。照片記錄 8 月 24 日起的校內團練、進音樂廳彩排與晚間正式登台，檔名留下的「Blue Midnight」也是曲目線索；主題名稱、指揮與完整節目單仍待補齊。',
...
function missingFields(concert) {
  ...
  if (!concert.performers || !concert.performers.length) missing.push('團員名單');
  return missing;
}
...
function introText(concert) {
  if (ARCHIVE_INTROS[concert.id]) return escapeHtml(expandArchiveIntro(concert, ARCHIVE_INTROS[concert.id]));
  const missing = missingFields(concert);
  const sentences = [];
